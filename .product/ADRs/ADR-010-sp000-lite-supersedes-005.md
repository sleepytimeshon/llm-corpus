...
**Model substitution rationale**: ADR-005 spec'd `llama3.1:8b-instruct` + `qwen2.5:7b-instruct` based on Berkeley Function Calling Leaderboard data current as of Stage 3 Validate (2026-04-27). `qwen3:8b` is in the Qwen lineage (8B parameter class) but is **not** the FCL-cited model — qwen3 introduced thinking-mode toggling and different tool-use training relative to Qwen 2.5. SP-000-lite therefore measures `qwen3:8b` on its own merits, NOT as a proxy for ADR-005's evidence base. The substitution is a cost-driven scope reduction (already-pulled model; no ~9 GB download blocking sprint start), not an equivalence claim. Single-model coverage in SP-000-lite (vs ADR-005's two-model coverage) is the primary scope reduction; covering both Llama and Qwen families is deferred to a follow-up SP-000-extended run if SP-000-lite signals warrant it.
...
- **ADR-005 amendment**: ADR-005's frontmatter `superseded_by` field is set to `ADR-010` in the same commit as this ADR's authoring. **ADR-010 establishes the project's supersession convention: superseded ADRs retain `status: accepted` as historical record with `superseded_by` populated, rather than flipping to `status: superseded`. ADR-001 through ADR-009 had `supersedes: null` and `superseded_by: null`; ADR-010 is the first application of this convention.** The MADR / Nygard tradition supports either retain-accepted or flip-to-superseded; this project chooses retain-accepted because the audit trail benefit (superseded ADR's `accepted` history is preserved in the file rather than relying on `git log` archaeology) outweighs the slight ambiguity at status-field-level (which `superseded_by` resolves).
