...
- **Telemetry-write IO failure mid-iteration (e.g., disk-full ENOSPC on JSONL append, fs-level failure on summary atomic-write)**: The pilot harness MUST emit an error-severity event to `Paths.telemetry()` (the production telemetry stream, NOT the pilot stream that just failed) and halt the iteration. The iteration is unsalvageable because SC-003's "exactly 50 events per iteration" invariant cannot be re-established mid-run. A re-run starts iteration 1 fresh; any partial `pilot-iter{N}.jsonl` content from the failed run is left in place for forensic inspection (per FR-PILOT-014's no-overwrite/no-delete invariant). The `retrieval_outcome` field is bounded at the Zod schema layer (`z.string().max(1024)`) so per-event JSONL lines stay under Constitution Principle IX's 4 KB POSIX-atomic boundary.
