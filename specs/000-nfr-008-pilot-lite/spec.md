...
- **LLM emits malformed tool call (schema violation on `corpus.find` arguments)**: Counted as a non-invocation in the headline N (single-shot; no retry). The telemetry event records `tool_invoked=false`, `tool_arguments_valid=false`, AND `malformed_call_payload` capturing the raw malformed arguments for later prompt-template diagnosis. The per-iteration summary reports malformed-call rate as a secondary diagnostic; a soft-threshold flag fires above 10/30 in the knowledge-grounded bucket but does NOT force escalation (binary exit remains parameterized on N alone per ADR-010).
