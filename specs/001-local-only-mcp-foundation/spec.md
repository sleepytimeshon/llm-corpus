...
- **SC-008 — Always-on enforcement (SP-001 partial; SP-005 full)**: SP-001 verifies (a) the egress hook is installed exactly once before any pipeline-package import (bootstrap-order test, distinct from SC-007 in scope), (b) the `egress.checkpoint` event-emission helper is exported from `@llm-corpus/contracts/telemetry` and importable from any future package (proving the import-binding is live and ready), and (c) a 10-document smoke fixture using the find-path emits one `egress.checkpoint` per document. **The "always-on across all five pipeline stages × N documents" verification is explicitly deferred to SP-005**, which re-runs SC-008 against the real ingest → classify → embed → index → find pipeline. SP-001's partial verification proves the *plumbing*; SP-005's verification proves the *coverage*. This honest partition replaces the prior "stub fixtures simulating future stages" framing per Architect critique 2026-05-05.
